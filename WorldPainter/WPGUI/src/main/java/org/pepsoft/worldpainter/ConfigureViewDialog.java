...
public class ConfigureViewDialog extends WorldPainterDialog implements WindowListener {
    ...
    private BufferedImage loadImage(File file) {
        if (file.isFile() && file.canRead()) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(this, "在导入这张图片时出现了错误。\n这可能不是有效的或者被支持的图片文件，或者文件本身损坏。", "导入图片出错", JOptionPane.ERROR_MESSAGE);
            } catch (RuntimeException | Error e) {
                logger.error(e.getClass().getSimpleName() + " while loading image " + file ,e);
                JOptionPane.showMessageDialog(this, "在导入这张图片时出现了错误。\n可能没有足够的内存，或者图片过大。", "导入图片出错", JOptionPane.ERROR_MESSAGE);
            }
        }
        return null;
    }
    ...
    static BufferedImage scaleImage(BufferedImage image, GraphicsConfiguration graphicsConfiguration, int scale) {
        try {
            ...
            JOptionPane.showMessageDialog(null, "在" + ((scale == 100) ? "优化" : "缩放") + "这张覆盖图时出现了错误。\n可能没有足够的内存，或者图片过大。", "Error " + ((scale == 100) ? "Optimising" : "Scaling") + " Image", JOptionPane.ERROR_MESSAGE);
            return null;
        }
    }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("网格大小：");

        checkBoxImageOverlay.setText("覆盖图片");
        ...
        jLabel12.setText("背景颜色：");

        checkBoxBackgroundImage.setText("背景图片");
        ...
        checkBoxShowBiomes.setText("显示Minecraft生物群系（如适用）");
        ...
        buttonFitToDimension.setText("适应维度尺寸");
