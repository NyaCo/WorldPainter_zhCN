...
public class ExportWorldDialog extends WorldPainterDialog {
    ...
    public ExportWorldDialog(java.awt.Frame parent, World2 world, BiomeScheme biomeScheme, ColourScheme colourScheme, CustomBiomeManager customBiomeManager, Collection<Layer> hiddenLayers, boolean contourLines, int contourSeparation, TileRenderer.LightOrigin lightOrigin, WorldPainter view) {
        ...
        if (selectedTiles != null) {
            radioButtonExportSelection.setText("导出" + selectedTiles.size() + "个选定区块");
...
//            setControlStates();
    ...
    private void export() {
        ...
        if (! new File(fieldDirectory.getText().trim()).isDirectory()) {
            ...
            JOptionPane.showMessageDialog(this, "选中的导出目录不存在或者不是一个目录。", "错误", JOptionPane.ERROR_MESSAGE);
            return;
        }
        if (fieldName.getText().trim().isEmpty()) {
            fieldName.requestFocusInWindow();
            Toolkit.getDefaultToolkit().beep();
            JOptionPane.showMessageDialog(this, "你还没有命名这个地图。", "错误", JOptionPane.ERROR_MESSAGE);
            return;
        }
        if ((! radioButtonExportEverything.isSelected()) && ((selectedTiles == null) || selectedTiles.isEmpty())) {
            ...
            Toolkit.getDefaultToolkit().beep();
            JOptionPane.showMessageDialog(this, "未选中导出的区块。", "错误", JOptionPane.ERROR_MESSAGE);
            return;
        }
        if ((comboBoxGenerator.getSelectedItem() == Generator.CUSTOM) && ((generatorOptions == null) || generatorOptions.trim().isEmpty())) {
            ...
            Toolkit.getDefaultToolkit().beep();
            JOptionPane.showMessageDialog(this, "The custom world generator name has not been set.\nUse the [...] button to set it.", "错误", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Check for warnings
        StringBuilder sb = new StringBuilder("<html>请确认即使有如下警告，<br>你也要导出这个世界：<br><ul>");
        ...
        if ((! radioButtonExportSelection.isSelected()) || (selectedDimension == DIM_NORMAL)) {
            ...
            if ((generator == Generator.FLAT) && (surfacePropertiesEditor.isPopulateSelected() || world.getDimension(DIM_NORMAL).getAllLayers(true).contains(Populate.INSTANCE))) {
                sb.append("<li>当前世界类型为超平坦而且使用了默认资源填充。<br>Minecraft<em>不会</em>用默认资源填充超平坦地图的任何区块。");
                showWarning = true;
            }
        }
        if (radioButtonExportSelection.isSelected()) {
            ...
            sb.append("<li>已选择导出的区块！只有" + dim + "维度的" + selectedTiles.size() + "个区块会被导出。");
        ...
        if (disabledLayerCount > 0) {
            if (disabledLayerCount == 1) {
                sb.append("<li>存在无效化的自定义覆盖层！<br>1个覆盖层不会被导出。");
            } else {
                sb.append("<li>存在无效化的自定义覆盖层！<br>" + disabledLayerCount + "个覆盖层不会被导出。");
            }
            showWarning = showWarning || (! disableDisabledLayersWarning);
        }
        sb.append("</ul>你确定要继续导出吗？</html>");
        if (showWarning && (JOptionPane.showConfirmDialog(this, sb.toString(), "回顾警告信息", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE) != JOptionPane.YES_OPTION)) {
            return;
        }
    ...
    private void initComponents() {
        ...
        jTabbedPane1.addTab("主世界", surfacePropertiesEditor);
        jTabbedPane1.addTab("主世界天花板", surfaceCeilingPropertiesEditor);
        jTabbedPane1.addTab("下界", netherPropertiesEditor);
        jTabbedPane1.addTab("下界天花板", netherCeilingPropertiesEditor);
        jTabbedPane1.addTab("末地", endPropertiesEditor);
        jTabbedPane1.addTab("末地天花板", endCeilingPropertiesEditor);

        checkBoxGoodies.setSelected(true);
        checkBoxGoodies.setText("生成奖励箱");
        ...
        radioButtonExportSelection.setText("导出选中区块");
        ...
        labelSelectTiles.setText("<html><u>选择区块</u></html>");
        ...
        checkBoxMapFeatures.setText("<html>生成建筑（<b><span style=\"color:#E53333;\">注意：可能会破坏地形</span></b>）</html>");
