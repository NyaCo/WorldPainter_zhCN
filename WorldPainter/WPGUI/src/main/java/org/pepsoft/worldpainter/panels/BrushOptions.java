...
public class BrushOptions extends javax.swing.JPanel implements Observer {
            ...
            if (filter.onlyOnObjectType != null) {
                switch (filter.onlyOnObjectType) {
                    ...
                    case ANNOTATION:
                        ...
                        buttonReplace.setText(Constants.COLOUR_NAMES[dataValue] + "标记层");
            ...
            if (filter.exceptOnObjectType != null) {
                switch (filter.exceptOnObjectType) {
                    ...
                    case ANNOTATION:
                        ...
                        buttonExceptOn.setText(Constants.COLOUR_NAMES[dataValue] + "标记层");
    ...
    private JPopupMenu createObjectSelectionMenu(final ObjectSelectionListener listener) {
        JMenuItem waterItem = new JMenuItem("水");
        waterItem.addActionListener(e -> listener.objectSelected(DefaultFilter.WATER, "水", null));
        JMenu popupMenu = new JMenu();
        popupMenu.add(waterItem);

        JMenuItem lavaItem = new JMenuItem("熔岩");
        lavaItem.addActionListener(e -> listener.objectSelected(DefaultFilter.LAVA, "熔岩", null));
        popupMenu.add(lavaItem);

        JMenuItem landItem = new JMenuItem("陆地");
        landItem.addActionListener(e -> listener.objectSelected(DefaultFilter.LAND, "陆地", null));
        popupMenu.add(landItem);
        
        JMenu terrainMenu = new JMenu("地表");
        JMenu customTerrainMenu = new JMenu("自定义");
        JMenu stainedClayTerrainMenu = new JMenu("染色粘土");
        ...
        for (Terrain terrain: Terrain.getConfiguredValues()) {
            ...
            } else if (terrain.getName().endsWith("黏土") && (terrain != Terrain.HARDENED_CLAY)) {
        ...
        final JMenu biomeMenu = new JMenu("生物群系");
        ...
        JMenu autoBiomeSubMenu = new JMenu("自动生物群系");
        JMenuItem autoBiomesMenuItem = new JMenuItem("所有自动生物群系");
        autoBiomesMenuItem.addActionListener(e -> listener.objectSelected(DefaultFilter.AUTO_BIOMES, "所有自动生物群系", null));
        ...
        JMenu annotationsMenu = new JMenu("标记层");
        JMenuItem menuItem = new JMenuItem("所有标记层");
        menuItem.addActionListener(e -> listener.objectSelected(new DefaultFilter.LayerValue(Annotations.INSTANCE), "所有标记层", null));
        ...
        for (int i = 1; i < 16; i++) {
            ...
            menuItem.addActionListener(e -> listener.objectSelected(new DefaultFilter.LayerValue(Annotations.INSTANCE, selectedColour), Constants.COLOUR_NAMES[dataValue] + "标记层", icon));
            annotationsMenu.add(menuItem);
        }
    ...
    private void initComponents() {
        ...
        checkBoxInSelection.setText("在选区内");
        ...
        checkBoxOutsideSelection.setText("在选区外");
