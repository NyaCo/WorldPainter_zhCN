...
outer:          for (int x = 0; x < width; x++) {
                ...
                labelImageDimensions.setText(String.format("图片尺寸：%d x %d，%d 位，最低值：%d，最高值：%d", width, height, bitDepth, imageLowValue, imageHighValue));
    ...
    private void updateWorldDimensions() {
        int scale = (Integer) spinnerScale.getValue();
        labelWorldDimensions.setText("显示尺寸：" + (image.getWidth() * scale / 100) + " x " + (image.getHeight() * scale / 100) + " 块");
    }
    ...
    private void resetDefaults() {
        if (currentDimension != null) {
            ...
            loadDefaults();
            buttonSaveAsDefaults.setEnabled(false);
            JOptionPane.showMessageDialog(this, "主题已恢复到出厂设置", "默认主题已被重置", JOptionPane.INFORMATION_MESSAGE);
        }
    }
    ...
    private void initComponents() {
        ...
        labelImageDimensions.setText("图片尺寸：? x ?，位深度：?，最低值：?，最高值：?");
        ...
        checkBoxInvert.setText("反相（高低倒置）");
        ...
        labelWorldDimensions.setText("显示尺寸：? x ? 块");
        ...
        jLabel2.setText("高度上限：");

        jLabel10.setText("块");

        checkBoxVoid.setText("在低于此高度的地区生成虚空");
        ...
        jLabel11.setText("坐标：");
        jLabel11.setToolTipText("灰度图的左上角位于地图的这个坐标");

        spinnerOffsetX.setModel(new javax.swing.SpinnerNumberModel(0, -999999, 999999, 1));
        spinnerOffsetX.setToolTipText("灰度图的左上角位于地图的这个坐标");
        ...
        jLabel12.setText(",");
        jLabel12.setToolTipText("灰度图的左上角位于地图的这个坐标");

        spinnerOffsetY.setModel(new javax.swing.SpinnerNumberModel(0, -999999, 999999, 1));
        spinnerOffsetY.setToolTipText("灰度图的左上角位于地图的这个坐标");
        ...
        checkBoxCreateTiles.setText("创建新区块");

        labelNoUndo.setText("<html><b>注意：</b>此操作无法撤销！</html>");

        checkBoxOnlyRaise.setText("是否只生成更高的部分");
        checkBoxOnlyRaise.setToolTipText("当被选中时，只有灰度图表示的高度比现在的地表高的部分会按照灰度图生成地形");

        jLabel13.setText("地图版本：");
