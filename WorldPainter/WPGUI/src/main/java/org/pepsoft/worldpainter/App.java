...
public final class App extends JFrame implements RadiusControl,
    ...
    public void setDimension(final Dimension dimension) {
        ...
        if (dimension != null) {
            setTitle("WorldPainter - " + world.getName() + " - " + dimension.getName() + " - 中华地形师协会定制版"); // NOI18N
...
//            }
            ...
            setTitle("WorldPainter 中华地形师协会定制版"); // NOI18N
...
//            selectionState.setValue(false);
    ...
    public void updateStatusBar(int x, int y) {
        ...
        if (tile == null) {
            ...
            if (dimension.isBorderTile(x >> TILE_SIZE_BITS, y >> TILE_SIZE_BITS)) {
                ...
                setTextIfDifferent(materialLabel, "Minecraft自生成");
            }
            setTextIfDifferent(biomeLabel, " ");
            return;
        }
        ...
        setTextIfDifferent(slopeLabel, MessageFormat.format("坡度：{0}°", (int) (Math.atan(dimension.getSlope(x, y)) * 180 / Math.PI + 0.5)));
            ...
            if (biome == 255) {
                ...
                if (biome != -1) {
                    if (biomeNames[biome] == null) {
                        setTextIfDifferent(biomeLabel, "自动生物群系：自动生物群系 " + biome);
                    } else {
                        setTextIfDifferent(biomeLabel, "自动生物群系：" + biomeNames[biome]);
                    }
                }
            } else if (biome != -1) {
                if (biomeNames[biome] == null) {
    ...
    void shiftWorld(Window parent) {
        if ((world.getImportedFrom() != null) && (showConfirmDialog(parent, "这是一个从现存地图导入的世界！\n如果你位移这个世界，你将无法准确地将其嵌入到原地图。\n你确定要位移这个世界吗？", strings.getString("imported"), YES_NO_OPTION, WARNING_MESSAGE) != YES_OPTION)) {
            return;
        }
...
//        popupMenu.add(label);
        ...
        if (customMaterials.length > 0) {
            JMenu existingMaterialsMenu = new JMenu("选择现存地表材料");
        ...
        if (button != null) {
            menuItem = new JMenuItem(((material != null) ? "编辑" : strings.getString("select.custom.material")) + "...");
            menuItem.addActionListener(e -> {
        ...
        menuItem = new JMenuItem("通过文件导入自定义地表层...");
        menuItem.addActionListener(e -> {
            if (button != null) {
    ...
    private void addButtonForNewCustomTerrain(int index, MixedMaterial customMaterial, boolean select) {
        ...
        if (customTerrainPanel == null) {
            dockingManager.addFrame(new DockableFrameBuilder(createCustomTerrainPanel(), "自定义地表", DOCK_SIDE_WEST, 3).withId("customTerrain").build());
        }
    ...
    public void customBiomeRemoved(CustomBiome customBiome) {
        ...
        if ((! programmaticChange) && (dimension != null)) {
            ProgressDialog.executeTask(this, new ProgressTask<Void>() {
                @Override
                public String getName() {
                    return "正在移除自定义生物群系" + customBiome.getName();
                }
    ...
    private boolean save(File file) {
        ...
        try {
            ...
            if (worldName.length() != name.length()) {
                world.setName(name);
                setTitle("WorldPainter - " + name + " - " + dimension.getName() + " - 中华地形师协会定制版"); // NOI18N
            } else {
                for (int i = 0; i < name.length(); i++) {
                    if ((name.charAt(i) != '_') && (name.charAt(i) != worldName.charAt(i))) {
                        world.setName(name);
    ...
    private void initComponents() {
        ...
        dockingManager.addFrame(new DockableFrameBuilder(createToolPanel(), "工具", DOCK_SIDE_WEST, 1).build());
        ...
        dockingManager.addFrame(new DockableFrameBuilder(createTerrainPanel(), "地表", DOCK_SIDE_WEST, 3).build());

        biomesPanel = new DockableFrameBuilder(createBiomesPanel(), "生物群系", DOCK_SIDE_WEST, 3).build();
        dockingManager.addFrame(biomesPanel);

        dockingManager.addFrame(new DockableFrameBuilder(createAnnotationsPanel(), "标记层", DOCK_SIDE_WEST, 3).build());

        dockingManager.addFrame(new DockableFrameBuilder(createBrushPanel(), "笔刷", DOCK_SIDE_EAST, 1).build());

        if (customBrushes.containsKey(CUSTOM_BRUSHES_DEFAULT_TITLE)) {
            dockingManager.addFrame(new DockableFrameBuilder(createCustomBrushPanel(CUSTOM_BRUSHES_DEFAULT_TITLE, customBrushes.get(CUSTOM_BRUSHES_DEFAULT_TITLE)), "自定义笔刷", DOCK_SIDE_EAST, 1).withId("customBrushesDefault").build());
        }
        ...
        dockingManager.addFrame(new DockableFrameBuilder(createBrushSettingsPanel(), "笔刷设置", DOCK_SIDE_EAST, 2).withId("brushSettings").build());

        dockingManager.addFrame(new DockableFrameBuilder(createInfoPanel(), "信息", DOCK_SIDE_EAST, 2).withId("infoPanel").expand().withIcon(loadScaledIcon("information")).build());
        ...
        if (config.getShowCalloutCount() > 0) {
            ...
            view.addOverlay("callout_1", 0, dockingManager.getFrame("工具"), callout);
            callout = loadCallout("callout_2");
            view.addOverlay("callout_2", -callout.getWidth(), dockingManager.getFrame("笔刷"), callout);
            callout = loadCallout("callout_3");
            view.addOverlay("callout_3", 0, dockingManager.getFrame("覆盖层"), callout);
    ...
    private JPanel createStatusBar() {
        ...
        Configuration config = Configuration.getInstance();
        if (config.isAutosaveEnabled() && config.isAutosaveInhibited()) {
            warnings.append("自动保存不可用");
        }
        if (config.isSafeMode()) {
            if (warnings.length() > 0) {
                warnings.append(": ");
            }
            warnings.append("安全模式");
        }
        if (warnings.length() > 0) {
        ...
        biomeLabel = new JLabel("自动生物群系：红木山丘（ID 161）");
        ...
        JLabel starCore = new JLabel("<html><font color=\"Orange\">【星核汉化组】</font></html>");
        starCore.setBorder(new BevelBorder(BevelBorder.LOWERED));
        statusBar.add(starCore);
...
//            selectionState.setValue(false);
        ...
        clearSelectionButton.setToolTipText("清除选区");
    ...
    private JPanel createBiomesPanel() {
        ...
        constraints.weightx = 0.0;
        biomesCheckBox = new JCheckBox("显示：");
        ...
        biomesSoloCheckBox = new JCheckBox("仅显示：");
    ...
    private JPanel createAnnotationsPanel() {
        ...
        constraints.anchor = GridBagConstraints.FIRST_LINE_START;
        constraints.weightx = 0.0;
        annotationsCheckBox = new JCheckBox("显示：");
        ...
        annotationsSoloCheckBox = new JCheckBox("仅显示：");
    ...
    private JPopupMenu createCustomLayerMenu(final String paletteName) {
        ...
        menuItem = new JMenuItem("添加自定义洞穴层...");
        menuItem.addActionListener(e -> {
        ...
        menuItem = new JMenuItem("通过文件导入自定义层...");
        menuItem.addActionListener(e -> importLayers(paletteName));
        customLayerMenu.add(menuItem);

        menuItem = new JMenuItem("从其他世界导入自定义层...");
    ...
    private JPanel createTerrainPanel() {
        ...
        constraints.anchor = GridBagConstraints.FIRST_LINE_START;
        constraints.weightx = 0.0;
        JCheckBox checkBoxShowTerrain = new RemoteJCheckBox(terrainCheckBox, "显示：");
        ...
        JCheckBox checkBoxSoloTerrain = new RemoteJCheckBox(terrainSoloCheckBox, "仅显示：");
    ...
    private JPanel createBrushPanel() {
        ...
        int brushRow = myConfig.getBrushRow();
        int brushColumn = myConfig.getBrushColumn();
        if (brushRow != 0 && brushColumn >= 1){
                brushRow = 0;
                myConfig.setBrushRow((Integer) brushRow);
            }
        if (brushRow <= 0 && brushColumn <= 0){
                brushRow = 0;
                brushColumn = 3;
                myConfig.setBrushRow((Integer) brushRow);
                myConfig.setBrushColumn((Integer) brushColumn);
            }
        JPanel optionsPanel = new JPanel();
        optionsPanel.setLayout(new GridBagLayout());
        JPanel brushPanel = new JPanel(new GridLayout(brushRow, brushColumn));
    ...
    private JPanel createCustomBrushPanel(String title, List<Brush> customBrushes) {
        ...
        JPanel customBrushPanel = new JPanel(new GridLayout(brushRow, brushColumn));
    ...
    private JPanel createBrushSettingsPanel() {
        ...
        levelSlider.addChangeListener(e -> {
            ...
            levelLabel.setText("笔刷硬度：" + ((value < 52) ? (value - 1) : value) + " %");
        ...
        brushRotationSlider.addChangeListener(e -> {
            ...
            brushRotationLabel.setText("旋转笔刷：" + ((value < 0) ? (((value - 7) / 15) * 15) : (((value + 7) / 15) * 15)) + "°");
        ...
        brushRotationLabel = new JLabel("旋转笔刷：0°");
        ...
        constraints.fill = GridBagConstraints.NONE;
        constraints.insets = new Insets(3, 1, 1, 1);
        brushSettingsPanel.add(new JLabel("设置"), constraints);
...
//            }
    ...
    public List<Component> createCustomLayerButton(final CustomLayer layer) {
        ...
        button.setToolTipText(button.getToolTipText() + "；右键展开选项");
        button.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            private void showPopup(MouseEvent e) {
                ...
                JMenu paletteMenu = new JMenu("移动到其他栏");
            ...
            private void edit() {
                ...
                if (!dialog.isCancelled()) {
                    ...
                    button.setToolTipText(layer.getName() + "：" + layer.getDescription() + "；右键展开选项");
    ...
    private JMenu createFileMenu() {
        ...
        recentMenu = new JMenu("恢复上次打开的世界");
        ...
        if (! config.isEasyMode()) {
            ...
            menuItem.setText("从Minecraft地图导入...");
        ...
        if (config.isEasyMode()) {
            ...
            exportHighResHeightMapMenuItem = new JMenuItem("导出为高分辨率灰度图...");
    ...
    private JMenu createEditMenu() {
        ...
        extendedBlockIdsMenuItem = new JCheckBoxMenuItem("查看方块ID");
        ...
        JMenu dimensionsMenu = new JMenu("维度");

        addSurfaceCeilingMenuItem = new JMenuItem("添加主世界天花板...");
        addSurfaceCeilingMenuItem.addActionListener(e -> addSurfaceCeiling());
        dimensionsMenu.add(addSurfaceCeilingMenuItem);

        removeSurfaceCeilingMenuItem = new JMenuItem("移除主世界天花板...");
        ...
        menuItem = new JMenuItem("从其他工程文件导入自定义层...");
        menuItem.setMnemonic('i');
        menuItem.addActionListener(e -> {
        ...
        menuItem.setText("从Layer文件导入自定义覆盖层...");
        importMenu.add(menuItem);

        menuItem = new JMenuItem("从Terrain文件导入自定义地表...");
        menuItem.setMnemonic('t');
        menuItem.addActionListener(e -> importCustomMaterials());
        importMenu.add(menuItem);

        menuItem = new JMenuItem("导入灰度图到当前维度...");
        menuItem.addActionListener(e -> importHeightMapIntoCurrentDimension());
        importMenu.add(menuItem);

        menuItem = new JMenuItem("导入地表或覆盖层蒙版...");
...
//        importMenu.add(menuItem);
        ...
        menuItem = new JMenuItem("移除未使用的层...");
...
//        menu.add(easyModeItem);
    ...
    private JMenu createViewMenu() {
        ...
        viewSurfaceCeilingMenuItem = new JCheckBoxMenuItem("查看主世界天花板", false);
        ...
        viewNetherCeilingMenuItem = new JCheckBoxMenuItem("查看下界天花板", false);
        ...
        viewEndCeilingMenuItem = new JCheckBoxMenuItem("查看末地天花板", false);
        ...
        colourSchemeMenu.add(new JLabel("弃用："));
        for (int i = 0; i < colourSchemeNames.length; i++) {
            if (deprecatedColourSchemes.contains(colourSchemeNames[i])) {
        ...
        JMenu workspaceLayoutMenu = new JMenu("工作空间布局");
...
//        menu.add(ACTION_SHOW_HELP_PICKER);
        ...
        menuItem = new JMenuItem(ACTION_OPEN_QQGROUP);
        menu.add(menuItem);
        menuItem = new JMenuItem(ACTION_OPEN_JAVADATA);
        menu.add(menuItem);
        menuItem = new JMenuItem(ACTION_OPEN_MCTIEBA);
        menu.add(menuItem);
        menuItem = new JMenuItem(ACTION_OPEN_WPTIEBA);
        menu.add(menuItem);

		menu.addSeparator();

        JMenu Update = new JMenu("检查最新汉化包更新数据");

        menuItem = new JMenuItem(ACTION_OPEN_9DAMAO);
        Update.add(menuItem);

        menuItem = new JMenuItem(ACTION_OPEN_MCBBS);
        Update.add(menuItem);

        menuItem = new JMenuItem(ACTION_OPEN_3DMGAME);
        Update.add(menuItem);

        menuItem = new JMenuItem(ACTION_OPEN_STEAMCN);
        Update.add(menuItem);
        menu.add(Update);
...
//            menu.addSeparator();
    ...
    private void removeSurfaceCeiling() {
        if (showConfirmDialog(this, "你确定要彻底删除主世界天花板吗？\n这个操作无法撤销！", "确认删除主世界天花板", YES_NO_OPTION) == YES_OPTION) {
            ...
            showMessageDialog(this, "主世界天花板被成功移除", "成功", INFORMATION_MESSAGE);
        }
    }
    ...
    private void removeNetherCeiling() {
        if (showConfirmDialog(this, "你确定要彻底删除下界天花板吗？\n这个操作无法撤销！", "确认删除下界天花板", YES_NO_OPTION) == YES_OPTION) {
            ...
            showMessageDialog(this, "下界天花板被成功移除", "成功", INFORMATION_MESSAGE);
        }
    }
    ...
    private void removeEndCeiling() {
        if (showConfirmDialog(this, "你确定要彻底删除末地天花板吗？\n这个操作无法撤销！", "确认删除末地天花板", YES_NO_OPTION) == YES_OPTION) {
            ...
            showMessageDialog(this, "末地天花板被成功移除", "成功", INFORMATION_MESSAGE);
        }
    }
...
//            @Override
//            public String getDescription() {
//                return "Minecraft 地图level文件 (level.dat)";
//            }
//        });
...
//        toolBar.add(ACTION_SHOW_HELP_PICKER);
    ...
    private void removeNether() {
        if (showConfirmDialog(this, "你确定要彻底删除下界吗？\n这个操作无法撤销！", "确认删除下界", YES_NO_OPTION) == YES_OPTION) {
            ...
            showMessageDialog(this, "下界被成功移除", "成功", INFORMATION_MESSAGE);
        }
    }
    ...
    private void removeEnd() {
        if (showConfirmDialog(this, "你确定要彻底删除末地吗？\n这个操作无法撤销！", "确认删除末地", YES_NO_OPTION) == YES_OPTION) {
            ...
            showMessageDialog(this, "末地被成功移除", "成功", INFORMATION_MESSAGE);
        }
    }
    ...
    private boolean importCustomMaterials() {
        ...
        if (customMaterials != null) {
            if (getConfiguredCustomMaterialCount() + customMaterials.length > CUSTOM_TERRAIN_COUNT) {
                showMessageDialog(this, "Not enough unused Custom Terrain slots available;\nselect " + (CUSTOM_TERRAIN_COUNT - Terrain.getConfiguredCustomMaterialCount()) + " Custom Terrains or fewer.", "过多自定义地表层", WARNING_MESSAGE);
                return false;
            }
            int nextIndex = 0;
    ...
    private void importLayers(String paletteName) {
        ...
        File[] selectedFiles = FileUtils.selectFilesForOpen(this, "选择WorldPainter覆盖层文件", layerDirectory, new FileFilter() {
            @Override
            public boolean accept(File f) {
        ...
        if (selectedFiles != null) {
            ...
            for (File selectedFile: selectedFiles) {
                try {
                    try (ObjectInputStream in = new ObjectInputStream(new GZIPInputStream(new BufferedInputStream(new FileInputStream(selectedFile))))) {
                        ...
                        for (Layer existingLayer : getCustomLayers()) {
                            if (layer.equals(existingLayer)) {
                                showMessageDialog(this, "这个覆盖层已经存在于此维度。\n这个覆盖层尚未使用。", "覆盖层已存在", WARNING_MESSAGE);
                                return;
                            }
                        }
                        if (paletteName != null) {
                        ...
                        if (layer instanceof CombinedLayer) {
                            ...
                            if (! combinedLayer.restoreCustomTerrain()) {
                                showMessageDialog(this, "这个覆盖层包含一个无法被还原的自定义地表层。该地表层已被重置。", "自定义地表无法还原", ERROR_MESSAGE);
                            } else {
                                // Check for a custom terrain type and if necessary make
                                // sure it has a button
                    ...
                    showMessageDialog(this, "指定路径不存在或不是一个文件", "不存在的文件", ERROR_MESSAGE);
                    return;
                } catch (IOException e) {
                    logger.error("I/O error while loading file " + selectedFile, e);
                    showMessageDialog(this, "读取指定文件时出现I/O错误或者不是一个有效的WorldPainter覆盖层文件", "I/O设备错误或无效的文件", ERROR_MESSAGE);
                    return;
                } catch (ClassNotFoundException e) {
                    logger.error("Class not found exception while loading file " + selectedFile, e);
                    showMessageDialog(this, "指定文件不是一个（有效的）WorldPainter覆盖层文件", "无效的文件", ERROR_MESSAGE);
                    return;
    ...
    private void exportLayer(CustomLayer layer) {
        ...
        File selectedFile = FileUtils.selectFileForSave(this, "导出WorldPainter覆盖层文件", new File(layerDirectory, FileUtils.sanitiseName(layer.getName()) + ".layer"), new FileFilter() {
            @Override
            public boolean accept(File f) {
                return f.isDirectory() || f.getName().toLowerCase().endsWith(".layer");
            }
        ...
        if (selectedFile != null) {
            ...
            if (selectedFile.isFile() && (showConfirmDialog(this, "文件" + selectedFile.getName() + "已存在。\n你想要覆盖这个文件吗？", "覆盖文件", YES_NO_OPTION) == NO_OPTION)) {
                return;
            }
            try {
            ...
            showMessageDialog(this, "覆盖层" + layer.getName() + "被成功导出", "成功", INFORMATION_MESSAGE);
        }
    }
    ...
    private void importCustomItemsFromWorld(CustomItemsTreeModel.ItemType itemType) {
        ...
        File selectedFile = FileUtils.selectFileForOpen(this, "选择一个WorldPainter世界", dir,
                new FileFilter() {
                    @Override
                    public boolean accept(File f) {
                        return f.isDirectory()
        ...
        if (selectedFile != null) {
            if (! selectedFile.isFile()) {
                ...
                showMessageDialog(this, "指定路径不存在或不是一个文件", "文件不存在", ERROR_MESSAGE);
                return;
            }
            if (! selectedFile.canRead()) {
                showMessageDialog(this, "WorldPainter没有权限读取选中的文件", "拒绝访问", ERROR_MESSAGE);
                return;
            }
            final World2 selectedWorld = ProgressDialog.executeTask(this, new ProgressTask<World2>() {
                ...
                private void logMetadataAsError(Map<String, Object> metadata) {
                    StringBuilder sb = new StringBuilder("world文件的元数据：\n");
                    appendMetadata(sb, metadata);
                    logger.error(sb.toString());
                }
                ...
                private void reportUnloadableWorldException(UnloadableWorldException e) {
                    try {
                        String text;
                        if (e.getMetadata() != null) {
                            StringBuilder sb = new StringBuilder("WorldPainter无法加载该文件，可能是因为：\n" +
                                    "\n" +
                                    "* 文件被损坏\n" +
                                    "* 文件创建于一个新版本的WorldPainter\n" +
                                    "* 文件使用了你没有的WorldPainter插件\n" +
                                    "\n");
                            appendMetadata(sb, e.getMetadata());
                            text = sb.toString();
                        } else {
                            text = "WorldPainter无法加载该文件，可能是因为：\n" +
                                    "\n" +
                                    "* 文件不是WorldPainter的world文件\n" +
                                    "* 文件被损坏\n" +
                                    "* 文件创建于一个新版本的WorldPainter\n" +
                                    "* 文件使用了你没有的WorldPainter插件";
                        }
            ...
            if (CustomItemsTreeModel.hasCustomItems(selectedWorld, itemType)) {
                ...
                if (! dialog.isCancelled()) {
                    ...
                    if (errors.length() > 0) {
                        JOptionPane.showMessageDialog(App.this, "部分物品没有被导入：\n\n" + errors, "部分物品未导入", showError ? JOptionPane.ERROR_MESSAGE : JOptionPane.WARNING_MESSAGE);
                    }
                }
            } else {
                String what;
                ...
                showMessageDialog(this, "选中的世界没有自定义" + what + "。", "自定义物品不存在", WARNING_MESSAGE);
            }
        }
    }
    ...
    private void deleteUnusedLayers() {
        ...
        if (unusedLayers.isEmpty()) {
            JOptionPane.showMessageDialog(this, "这个维度没有未使用的覆盖层。", "没有未使用的层", JOptionPane.INFORMATION_MESSAGE);
        } else {
            DeleteLayersDialog dialog = new DeleteLayersDialog(this, unusedLayers);
            dialog.setVisible(true);
            if (! dialog.isCancelled()) {
                JOptionPane.showMessageDialog(this, "选中的覆盖层已被删除。", "覆盖层已被删除", JOptionPane.INFORMATION_MESSAGE);
            }
        }
    }
    ...
    private final BetterAction ACTION_IMPORT_MAP = new BetterAction("importMinecraftMap", strings.getString("existing.minecraft.map") + "...", false) {
        {
            ...
            setShortDescription("从现存的Minecraft地图导入地形。使用嵌入功能以将你的更改嵌入到原地图。");
        }
    ...
    private final BetterAction ACTION_MERGE_WORLD = new BetterAction("mergeWorld", strings.getString("merge.world") + "...", false) {
        {
            ...
            setShortDescription("将在之前导入的地图上进行的更改嵌入到原地图中。");
        }
...
//            view.moveTo(location);
    ...
    private final BetterAction ACTION_SHIFT_WORLD = new BetterAction("shift", "位移...", ICON_SHIFT_WORLD) {
        {
            setShortDescription("位移");
        }
    ...
    private final BetterAction ACTION_OPEN_QQGROUP = new BetterAction("browseDocumentation", "Minecraft中华地形师协会") {
        @Override
        public void performAction(ActionEvent event) {
            try {
                DesktopUtils.open(new URL("https://jq.qq.com/?_wv=1027&k=5dvq3Zv"));
            } catch (MalformedURLException e) {
                throw new RuntimeException(e);
            }
        }
    ...
    private final BetterAction ACTION_OPEN_JAVADATA = new BetterAction("browseDocumentation", "中文Minecraft Wiki") {
        @Override
        public void performAction(ActionEvent event) {
            try {
                DesktopUtils.open(new URL("https://minecraft-zh.gamepedia.com/Minecraft_Wiki"));
            } catch (MalformedURLException e) {
                throw new RuntimeException(e);
            }
        }
    ...
    private final BetterAction ACTION_OPEN_MCTIEBA = new BetterAction("browseDocumentation", "百度Minecraft贴吧") {
        @Override
        public void performAction(ActionEvent event) {
            try {
                DesktopUtils.open(new URL("https://tieba.baidu.com/f?kw=minecraft&ie=utf-8"));
            } catch (MalformedURLException e) {
                throw new RuntimeException(e);
            }
        }
    ...
    private final BetterAction ACTION_OPEN_WPTIEBA = new BetterAction("browseDocumentation", "百度WorldPainter贴吧") {
        @Override
        public void performAction(ActionEvent event) {
            try {
                DesktopUtils.open(new URL("https://tieba.baidu.com/f?kw=worldpainter&ie=utf-8"));
            } catch (MalformedURLException e) {
                throw new RuntimeException(e);
            }
        }
    ...
    private final BetterAction ACTION_OPEN_9DAMAO = new BetterAction("browseDocumentation", "9damao 9dm玖大猫游戏mod论坛") {
        @Override
        public void performAction(ActionEvent event) {
            try {
                DesktopUtils.open(new URL("http://www.9damao.com/thread-163367-1-1.html"));
            } catch (MalformedURLException e) {
                throw new RuntimeException(e);
            }
        }
    ...
    private final BetterAction ACTION_OPEN_MCBBS = new BetterAction("browseDocumentation", "Minecraft(我的世界)中文论坛") {
        @Override
        public void performAction(ActionEvent event) {
            try {
                DesktopUtils.open(new URL("http://www.mcbbs.net/thread-854767-1-1.html"));
            } catch (MalformedURLException e) {
                throw new RuntimeException(e);
            }
        }
	...
	private final BetterAction ACTION_OPEN_3DMGAME = new BetterAction("browseDocumentation", "3DMGAME论坛") {
        @Override
        public void performAction(ActionEvent event) {
            try {
                DesktopUtils.open(new URL("http://bbs.3dmgame.com/thread-5871192-1-1.html"));
            } catch (MalformedURLException e) {
                throw new RuntimeException(e);
            }
        }
	...
	private final BetterAction ACTION_OPEN_STEAMCN = new BetterAction("browseDocumentation", "SteamCN 蒸汽动力") {
        @Override
        public void performAction(ActionEvent event) {
            try {
                DesktopUtils.open(new URL("https://steamcn.com/t479731-1-1"));
            } catch (MalformedURLException e) {
                throw new RuntimeException(e);
            }
        }
    ...
    private final BetterAction ACTION_IMPORT_LAYER = new BetterAction("importLayer", "导入自定义层") {
        @Override
    ...
    private final BetterAction ACTION_ROTATE_BRUSH_LEFT = new BetterAction("rotateBrushLeft", "将笔刷逆时针旋转15°") {
        @Override
        protected void performAction(ActionEvent e) {
    ...
    private final BetterAction ACTION_ROTATE_BRUSH_RIGHT = new BetterAction("rotateBrushRight", "将笔刷顺时针旋转15°") {
        @Override
        protected void performAction(ActionEvent e) {
            int rotation = brushRotationSlider.getValue() + 15;
            if (rotation > 180) {
    ...
    private final BetterAction ACTION_ROTATE_BRUSH_RESET = new BetterAction("rotateBrushReset", "将笔刷旋转重置为0°") {
        @Override
        protected void performAction(ActionEvent e) {
    ...
    private final BetterAction ACTION_ROTATE_BRUSH_RIGHT_30_DEGREES = new BetterAction("rotateBrushRight30Degrees", "将笔刷顺时针旋转30°") {
        @Override
        protected void performAction(ActionEvent e) {
    ...
    private final BetterAction ACTION_ROTATE_BRUSH_RIGHT_45_DEGREES = new BetterAction("rotateBrushRight45Degrees", "将笔刷顺时针旋转45°") {
        @Override
        protected void performAction(ActionEvent e) {
    ...
    private final BetterAction ACTION_ROTATE_BRUSH_RIGHT_90_DEGREES = new BetterAction("rotateBrushRight90Degrees", "将笔刷顺时针旋转90°") {
        @Override
        protected void performAction(ActionEvent e) {
    ...
    private final BetterAction ACTION_RESET_DOCKS = new BetterAction("resetDockLayout", "重置当前和默认布局") {
        @Override
        protected void performAction(ActionEvent e) {
            dockingManager.resetToDefault();
            Configuration config = Configuration.getInstance();
    ...
    private final BetterAction ACTION_RESET_ALL_DOCKS = new BetterAction("resetAllDockLayout", "重置当前、默认和所有世界的布局（慎点）") {
        @Override
        protected void performAction(ActionEvent e) {
            dockingManager.resetToDefault();
            Configuration config = Configuration.getInstance();
    ...
    private final BetterAction ACTION_LOAD_LAYOUT = new BetterAction("loadDockLayout", "读取工作空间布局") {
        @Override
        protected void performAction(ActionEvent e) {
            Configuration config = Configuration.getInstance();
            if (config.getDefaultJideLayoutData() != null) {
    ...
    private final BetterAction ACTION_SAVE_LAYOUT = new BetterAction("resetDocks", "保存工作空间布局") {
        @Override
        protected void performAction(ActionEvent e) {
            Configuration config = Configuration.getInstance();
    ...
    private final BetterAction ACTION_SWITCH_TO_FROM_CEILING = new BetterAction("switchCeiling", "切换到当前世界的天花板") {
        {
    ...
    private final BetterAction ACTION_SHOW_CUSTOM_TERRAIN_POPUP = new BetterAction("showCustomTerrainMenu", null, loadScaledIcon("plus")) {
        {
            setShortDescription("添加新的自定义地表层");
        }
...
//    private JScrollPane scrollPane = new JScrollPane();
    ...
    private static final String CUSTOM_BRUSHES_DEFAULT_TITLE = "自定义笔刷";
