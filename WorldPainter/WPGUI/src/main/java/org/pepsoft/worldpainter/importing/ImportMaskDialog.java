...
public class ImportMaskDialog extends WorldPainterDialog implements DocumentListener {
    ...
    private void loadImage() {
        try {
            ...
            if (image == null) {
                ...
                if (inputType == COLOUR) {
                    labelImageDimensions.setText(String.format("图片尺寸：%d x %d，indexed colour，%d位", width, height, image.getSampleModel().getSampleSize(0)));
                } else if (inputType == ONE_BIT_GRAY_SCALE) {
                    labelImageDimensions.setText(String.format("图片尺寸：%d x %d，black and white", width, height));
                } else {
                    labelImageDimensions.setText(String.format("图片尺寸：%d x %d，grey scale，%d位，最低值：%d，最高值：%d", width, height, image.getSampleModel().getSampleSize(0), maskImporter.getImageLowValue(), maskImporter.getImageHighValue()));
                }
    ...
    private void initComponents() {
        ...
        labelImageDimensions.setText("图片尺寸：? x ?，位深值：?，最低值：?，最高值：?");
        ...
        checkBoxRemoveExisting.setText("移除现存的覆盖层");
