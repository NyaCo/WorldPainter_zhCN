...
public class Bo2LayerEditor extends AbstractLayerEditor<Bo2Layer> implements ListSelectionListener, DocumentListener {
            ...
            if (selectedFiles.length > 0) {
                ...
                for (File selectedFile: selectedFiles) {
                    if (selectedFile.isDirectory()) {
                        ...
                        if (files.length == 0) {
                            JOptionPane.showMessageDialog(this, "目录" + selectedFile.getName() + "不包含任何有效的自定义模型文件。", "缺少自定义模型文件", JOptionPane.ERROR_MESSAGE);
                        } else {
                            for (File file: files) {
                                try {
                                    ...
                                    logger.error("I/O error while trying to load custom object " + file, e);
                                    JOptionPane.showMessageDialog(this, "加载" + file.getName() + "时出现了I/O错误；文件添加失败", "I/O错误", JOptionPane.ERROR_MESSAGE);
                                }
                            }
                        }
                    } else {
                        ...
                        try {
                            ...
                            JOptionPane.showMessageDialog(this, "加载" + selectedFile.getName() + "时出现了I/O错误；文件添加失败", "I/O错误", JOptionPane.ERROR_MESSAGE);
                        }
                    }
                }
                settingsChanged();
    ...
    private void reloadObjects() {
        ...
        if ((noFiles.length() > 0) || (notFound.length() > 0)) {
            ...
            JOptionPane.showMessageDialog(this, message, "部分文件加载失败", JOptionPane.ERROR_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(this, indices.length + "个模型被成功加载", "成功", JOptionPane.INFORMATION_MESSAGE);
        }
        refreshLeafDecaySettings();
    }
...
outer:  for (Enumeration<WPObject> e = (Enumeration<WPObject>) listModel.elements(); e.hasMoreElements(); ) {
        ...
        if (decayingLeavesFound) {
            if (nonDecayingLeavesFound) {
                ...
                labelLeafDecayTitle.setEnabled(true);
                labelEffectiveLeafDecaySetting.setEnabled(true);
                labelEffectiveLeafDecaySetting.setText("<html>树叶<b>可</b>枯萎。</html>");
                buttonSetDecay.setEnabled(false);
                buttonSetNoDecay.setEnabled(true);
                buttonReset.setEnabled(true);
            }
            ...
            if (nonDecayingLeavesFound) {
                ...
                labelLeafDecayTitle.setEnabled(true);
                labelEffectiveLeafDecaySetting.setEnabled(true);
                labelEffectiveLeafDecaySetting.setText("<html>树叶<b>不可</b>枯萎。</html>");
                buttonSetDecay.setEnabled(true);
                buttonSetNoDecay.setEnabled(false);
                buttonReset.setEnabled(true);
            } else {
    ...
    private void updateBlocksPerAttempt() {
        ...
        sb.append("（硬度为1%时，每").append(numberFormat.format(blocksAt1)).append("个方块放置一个模型");
        if (blocksAt100 <= 1) {
            sb.append("；硬度为100%时，每1个方块上放置一个模型）");
        } else {
            sb.append("；硬度为100%时，每").append(numberFormat.format(blocksAt100)).append("个方块上放置一个模型）");
        }
    ...
    private void initComponents() {
        ...
        labelLeafDecayTitle.setText("设置树叶是否可枯萎：");

        labelEffectiveLeafDecaySetting.setText("<html>树叶<b>不可</b>枯萎。</html>");
        labelEffectiveLeafDecaySetting.setEnabled(false);

        buttonSetDecay.setText("设置所有树叶为可枯萎");
        buttonSetDecay.setToolTipText("设置所有树叶为可枯萎");
        ...
        buttonSetNoDecay.setText("<html>设置所有树叶为<b>不可</b>枯萎</html>");
        buttonSetNoDecay.setToolTipText("设置所有树叶为不可枯萎");
        ...
        jLabel6.setText("<html><u>获取自定义模型（英文）</u></html>");
        ...
        jLabel1.setText("在这个界面设置你的自定义模型层。");
        ...
        jLabel9.setText("个方块上放置一个模型");

        labelBlocksPerAttempt.setText("（硬度为1%时，每1280个方块放置一个模型；硬度为100%时，每6个方块上放置一个模型）");

        jLabel10.setText("硬度为50%时，每");
