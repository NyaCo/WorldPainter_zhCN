...
public class ErrorDialog extends javax.swing.JDialog {
    ...
    public void setException(Throwable exception) {
        ...
        if (rootCause instanceof OutOfMemoryError) {
            setTitle("内存不足");
            jTextArea1.setText("没有足够的内存来执行这个操作！\n\n"
    ...
    private void email() {
        try {
            ...
            desktop.mail(uri);
            JOptionPane.showMessageDialog(this, "已自动新建一个电子邮件。\n如果没有弹出窗口，请使用“复制到剪贴板”按钮，\n手动将电子邮件发送到worldpainter@pepsoft.org。", "邮件已创建", JOptionPane.INFORMATION_MESSAGE);
        } catch (URISyntaxException e) {
            logger.error("URI syntax error while trying to send email", e);
            JOptionPane.showMessageDialog(this, "无法创建错误报告邮件！\n请使用“复制到剪贴板”按钮，\n将信息通过电子邮件发送到worldpainter@pepsoft.org。", "无法创建邮件", JOptionPane.ERROR_MESSAGE);
        } catch (IOException e) {
    ...
    private void copyToClipboard() {
        ...
        JOptionPane.showMessageDialog(this, "信息已复制到剪贴板。\n请将其粘贴到电子邮件中并发送到worldpainter@pepsoft.org。", "信息已复制", JOptionPane.INFORMATION_MESSAGE);
    }
